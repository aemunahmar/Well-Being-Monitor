<?php
    ...
    $sql = "INSERT INTO approve (patientid, doctorid, lname, fname, insurance, date, medicine, medication, cause, strength, dosage, allergies, notes) SELECT patientid, doctorid, lname, fname, insurance, date, medicine, medication, cause, strength, dosage, allergies, notes FROM prescription WHERE patientid = $patientid";
    $sql2 = "DELETE FROM prescription WHERE patientid = $patientid";
    $sql3 = "UPDATE approve SET pharmacistid='$pharmID' WHERE patientid = $patientid";
    ...
    if (($dbconnect->query($sql) === TRUE) && ($dbconnect->query($sql2) === TRUE)  && ($dbconnect->query($sql3) === TRUE)) 
    {
        ...
        //echo "Prescription for patient: " . $patientid. " has been approved.";
    } else
    {
